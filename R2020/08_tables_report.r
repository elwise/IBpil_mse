...
df <- read.table(file.path(res.dir,"stats2020.csv"), header=T, sep=";")
...
aux <- df %>% subset(Rule %in% c("HCR8") & Ass=="ASSnone") %>%
  group_by(Rec, catch, Rule) %>%
...
write.table(t(aux), file=file.path(res.dir, "table_report_HCR8_ASSnone.csv"), sep=";", col.names=F)

# tables for the report WITH ASSESSMENT

aux <- df %>% subset(Rule %in% c("HCR8") & Ass=="ASSss3") %>%
  group_by(Rec, catch, Rule) %>%
  summarise(round(Median_B1plus[period=="initial"]/1000, 0),
            round(Median_B1plus[period=="short"]/1000, 0),
            round(Median_B1plus[period=="last"]/1000, 0),
...
write.table(t(aux), file=file.path(res.dir, "table_report_HCR8_ASSss3.csv"), sep=";", col.names=F)
