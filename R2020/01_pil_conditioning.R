...
#   created:  03/07/2018                                                       #
#   modified: Feb 2020 Laura Wise (assessment WGHANSA2020)                     #
################################################################################
...
#==============================================================================
# ASSUMPTIONS - COMPARISON TO WGHANSA2020 (i.e. the BC)                    ----
#==============================================================================

# - REFERENCE POINTS: taken WGHANSA2017 ones
# - PROJECTION PERIOD values:
#   ~ @mat     : mean 2014-2019 (0 for age 0 and 1 for the rest)
#   ~ @stock.wt: mean 2014-2019 
#   ~ @catch.wt: mean 2017-2019 
#   ~ @catch.q : mean 2014-2019 
...
proj.yr  <- 2021  # first year of projection
...
load(paste(wd_dat,"pil.stock2020.RData",sep=""))
...
# 
# indices01 <- read.table(file.path(wd_dat, "acousticSurveyAgeData2020.txt"))
# indices01 <- t(indices01[,c(10:16)])
# 
# indices02 <- read.table(file.path(wd_dat, "depmSurveyBiomassData2020.txt"))
# indices02 <- t(indices02[,4])
# 
# # transform into FLQuant
# indices01 <- FLQuant(as.matrix(indices01), dimnames=list(age=0:6, year = 1996:2020))
# indices02 <- FLQuant(as.matrix(indices02), dimnames=list(biomass='ssb', year = 1997:2020))
# 
# # and into FLIndex
...
# 
# 
# save(indices,file = paste(wd_dat,'pilFLindices2020.RData',sep=""))

load(paste(wd_dat,"pilFLindices2020.RData",sep=""))
...
pil@desc <- "WGHANSA2020 assessment output"
...
m(biols$PIL)[,ac(proj.yrs),] <- m(biols$PIL[,ac(2019)])

# - Weights at age in the stock: decreasing trend for all ages --> mean 2014-2019 (last 6 years)
myrs <- ac(2014:2019)
# statistics
...
# 2014-2019
# 0     1     2     3     4     5     6 
# 0.000 0.029 0.046 0.062 0.065 0.070 0.072 
...
# I've kept the same names as in previous MSE despite the fact that reference points have changed since
Fpa  <- 0.190 
Flim <- 0.250 
Bpa  <- 446331 
Blim <- 337448 
...
pilsr@rec[,ac(ass.yr),] <- NA  # remove REC estimate for assessment year
...
yrs.srmed <- 1993:2019
params_med <- fmle(window(pilsr, start=1993, end=2019), fixed=list(b=PIL_ref.pts[["Blim"]]))@params # for pilsr
...
pilsr2@rec[,ac(ass.yr),] <- NA  # remove REC estimate for assessment year

model(pilsr2) <- segreg()

yrs.srlow <- 2006:2019
params_low <- fmle(window(pilsr2, start=2006, end=2019), fixed=list(b=Blow))@params 
...
# - different Hockey - stick ( one with inflexion point at Blow and the other at Blim)
...
B <- biols$PIL@n*catch.q.wt*exp(-m(biols[["PIL"]])/2)
...
# 2017 - 2019
#     0     1     2     3     4     5     6 
#     0.027 0.045 0.060 0.075 0.085 0.091 0.097 
...
# - Catchability at age (Cobb Douglas): mean 2014-2019 (last 6 years)
myrs <- ac(2014:2019)
# statistics
PILqa.mean <- yearMeans(catch.q(fleets$INT@metiers$ALL@catches$PIL)[,myrs,])
PILqa.mean[drop=T]
# 0        1        2        3        4        5        6 
# 0.02291706 0.08135689 0.13204123 0.14107079 0.14080634 0.14074325 0.11504918

PILqa.var  <- yearVars(catch.q(fleets$INT@metiers$ALL@catches$PIL)[,myrs,])
PILqa.var[drop=T]
#0            1            2            3            4            5            6 
#0.0001451025 0.0017294716 0.0043165511 0.0048692256 0.0048459041 0.0048403292 0.0033219687 
PILqa.cv   <- sqrt(PILqa.var)/yearMeans(catch.q(fleets$INT@metiers$ALL@catches$PIL)[,myrs,])
PILqa.cv[drop=T]
#0         1         2         3         4         5         6 
#0.5256281 0.5111665 0.4975754 0.4946441 0.4943849 0.4943219 0.5009729 
...
flq <- flq.PIL <- FLQuant(dimnames = list( age = "all", year = first.yr:last.yr, unit = PIL.unit, 
...
assess0 <- SS_output(dir = paste(wd_dat,"assess_ref2000",sep=""), forecast=F,printstats=F,verbose=F)
datss<-SS_readdat(paste(wd_dat,"assess_ref2000/sardine.dat",sep=""),verbose=F)

estim_catchN<-subset(assess0$catage,Fleet==1&Yr>=2006&Yr<2020)[,ac(0:6)]
...
computed_qage<-(c(0,0.01481691, -0.08860246, -0.04917801,  0.14546705,  0.28624775, -0.14607796))

indices_ss3$AcousticNumberAtAge@index.q[,]<-exp(computed_qage)
indices_ss3$AcousticNumberAtAge@index.q[1,]<-0
indices_ss3$DEPM@index.q[] <- 1.1906
...
indices_ss3$DEPM@index.q[,ac(last_DEPM+2),]<-NA
...
#cvs at age
#linhas<-grep("NatAge_std_All_Fem_A",assess0$derived_quants$Label)
#pop<-data.frame(n=assess0$derived_quants[linhas,"Value"],cv=assess0$derived_quants[linhas,"StdDev"]/assess0$derived_quants[linhas,"Value"])
# pop
# n        cv
# 1 6160040 0.2146496
# 2 1423100 0.1917321
...
assess.ctrl <- list( PIL=list(assess.model="assessEmul", 
                              ...
                              control=list( cv.age = c(NA,0.21,0.19,0.20,0.16,0.17,0.18), # cvs at age
...
assess.ctrl_SS3 <- list(PIL=list(assess.model = "ss32flbeia", work_w_Iter = FALSE,harvest.units='f',
                             control=list(ref.name="ss3R",
                                          assess_dir="D:/IPMA/SARDINE/WGHANSA2020/SS3/2020_Update/")))
...
PIL_advice.TAC.flq[,ac(c(1987:1989,1991,1993,1999:2019)),] <- c(140000,150000,212000,176000,135000,38000,81000,
                                                                88000,95000,100000,128000,106000,96000,114000,
                                                                92000,71000,75000,75000,36000,55000,17000,16000,12000,23000, 0,0)  # ICES #advice
PIL_advice.TAC.flq[,ac(2020),] <- c(19106) # New catch estimate for 2020 (HCR12)

PIL_advice.quota.share.flq <- FLQuant( 1, dimnames=list(quant=stks,year=first.yr:last.yr), iter=ni)
PIL_advice.avg.yrs         <- c(2015:2019)
...
main.ctrl$sim.years["initial"] <- 2020
...
save( biols, 
      ...
      file=paste(wd_in,"PIL_input2020.RData",sep=""))

save( PIL_ref.pts, file=paste(wd_in,"PIL_refpts2020.RData",sep=""))
