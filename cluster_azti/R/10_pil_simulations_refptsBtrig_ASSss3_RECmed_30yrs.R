...
#   modified: Leire Ibaibarriaga for refpts
################################################################################
...
fscan.vec <- rep(c(seq(0, 0.5, by=0.01), seq(1, 2, by=0.5)), each=1000)
length(fscan.vec) # 54000 cases for the cluster: 54 values of fscan x 1000 iters per value

it.vec <- rep(1:1000, times=54)
length(it.vec) # 54000 cases for the cluster
...
# set values for fscan with HCR15 (ICES AR: fixed F with Btrigger) and iteration number (it)

fscan <- fscan.vec[task.id]
it <- it.vec[task.id]
...
#==============================================================================
  ...
  scenario <- paste("ASS",ass.sc,"_HCR",rule.sc,"_REC",rec.sc,"_INN",inn.sc,"_OER",oer.sc,"_Fscan",Fscan.sc,sep="")
  ...
  if (rule.sc==1 | rule.sc==3 | rule.sc==5) {
    ...
    advice.ctrl$PIL$ref.pts['Flim',] <- fscan
    
  } else if (rule.sc==15 | rule.sc==16) {
    
    if(any(!c("Bloss","Blim","Flow","Fmsy") %in% rownames(advice.ctrl$PIL$ref.pts)))
      stop("Required reference points missing: Bloss, Blim, Flow, Fmsy")
    
    advice.ctrl$PIL$ref.pts['Fmsy',] <- fscan
...
main.ctrl$sim.years["final"] <- 2050
...
out.file      <- file.path("output_refpts/output_iters",out.name)
...
if (it==1) {
  if (!file.exists(file.path("output_refpts","scenario_list.RData"))) {
    scenario_list <- scenario
    save(scenario_list, file=file.path("output_refpts","scenario_list.RData"))
  } else {
    load(file.path("output_refpts","scenario_list.RData"))
