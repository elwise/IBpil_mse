...
# Modified by Laura Wise Fev 2020
#-------------------------------------------------------------------------------
...
pilHCRs <- function(stocks, advice, advice.ctrl, year, season, stknm,...){
  ...
  if (rule %in% c(0:12)) {
    ...
    if (rule == 0) { # HCR0: ICES HCR (F-based HCR in absolute terms)
      ...
      # ad-hoc modification of rule parameters (although Blim and Fmsy are already correctly established in the conditioning)
      ref.pts['Fmsy',] <- 0.12
      ref.pts['Bmsy',] <- 446331
      ref.pts['Blim',] <- 337448
    ...
    }else if (rule == 7) { # HCR7: No fishing
      ...
      Brefs <- rbind(ref.pts['Bloss',],ref.pts['Bmsy',], ref.pts['Bpa',])
      ...
      for(i in 1:iter){
        ...
        if( (rule == 1 | rule==3 | rule==5) & Ftg[i]==ref.pts['Floss',]){
          fwd.ctrl <- fwdControl(data.frame(year = c(assyrnumb+1), quantity = c('f','ssb')))
          ...
          #Set f 
          fwd.ctrl@trgtArray[1,2,] <- Ftg[i]
          #Set maximum catch of 40000
          fwd.ctrl@trgtArray[2,3,] <- 40000
          
        } else if (rule ==9 ){
          fwd.ctrl <- fwdControl(data.frame(year = c(assyrnumb+1), quantity = c('f','catch')))
          #Set f 
          fwd.ctrl@trgtArray[1,2,] <- Ftg[i]
          #Set maximum catch of 45000
          fwd.ctrl@trgtArray[2,3,] <- 45000
